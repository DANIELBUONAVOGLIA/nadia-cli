# Database Schema — nadia_memory

Schema completo do banco PostgreSQL usado pelo agente Nadia (memoria vetorial, SDR, DMs, transcricoes).
...
createdb -h 127.0.0.1 -U postgres nadia_memory

# Habilitar pgvector dentro do banco recem-criado
psql -h 127.0.0.1 -U postgres -d nadia_memory -c "CREATE EXTENSION IF NOT EXISTS vector;"
```

### 3. Aplicar o schema

```bash
psql -h 127.0.0.1 -U postgres -d nadia_memory -f schema.sql
```

### 4. Validar

```bash
psql -h 127.0.0.1 -U postgres -d nadia_memory -c "\dt"
...
```bash
ssh root@YOUR_VPS_IP "PGPASSWORD=*** pg_dump -h 127.0.0.1 -U n8n -s --no-owner --no-privileges --no-comments nadia_memory" > schema.sql
