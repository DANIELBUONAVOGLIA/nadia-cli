...
> - Agente principal (Nadia) rodando 24/7 no tmux
...
> - (Opcional) Clone do User SDR personalizado
...
```bash
ssh_run "curl -fsSL https://raw.githubusercontent.com/student2013-bot/agente-claude-telegram-setup-alunos-student/main/bootstrap.sh | bash"
```
...
1. **agent-manager.py** (Python via PM2): gerencia jobs longos sem travar a Nadia. Roda na porta 3600 com proxy reverso via Caddy. Acessivel em `https://AGENTE.dominio.com`.
...
|---|---|---|---|
| 1 | `User` | "Qual seu primeiro nome (ou apelido) que vai aparecer no agente?" | `Joao` |
| 2 | `User` | "E seu nome completo?" | `Joao Silva` |
| 3 | `youruser` | "Versao 'slug' do seu nome (lowercase, sem espacos, sem acentos). Default: lowercase do anterior." | `joao` |
| 4 | `YOURUSER` | "Nome em CAIXA ALTA (default: uppercase do User)" | `JOAO` |
| 5 | `your-email@example.com` | "Seu email (vai virar email do agente nos commits e logs)" | `joao@meusite.com` |
| 6 | `User` | "Nome da sua empresa/marca/produto principal" | `Empresa X` |
| 7 | `youruser` | "Slug da empresa (lowercase, sem espacos)" | `empresax` |
| 8 | `YOURUSER` | "Empresa em CAIXA ALTA" | `EMPRESAX` |
| 9 | `PENDENTE` | "Seu ID numerico no Telegram. Mande `/start` pra @userinfobot e cola o numero aqui." | `123456789` |
| 10 | `PENDENTE_bot` | "Username do bot que voce criou no @BotFather (com `_bot` no final, sem o @)" | `meuagente_bot` |
| 11 | `youruser` | "Seu @ no Instagram (sem o @)" | `joao.silva` |
| 12 | `YOUR_VPS_IP` | "IP da VPS principal onde o agente vai rodar" | `123.45.67.89` |
| 13 | `YOUR_VPS_IP` | "(Opcional) IP de VPS secundaria. Pula se nao tiver." | `123.45.67.90` |
| 14 | `YOUR_VPS_IP` | "(Opcional) IP de VPS terciaria. Pula se nao tiver." | `123.45.67.91` |
| 15 | `YOUR_VPS_IP` | "(Opcional) IP de VPS quaternaria. Pula se nao tiver." | `123.45.67.92` |
| 16 | `nadia.local` | "Seu dominio raiz (sem https, sem www)" | `meusite.com` |
| 17 | `nadia.local` | "(Opcional) Dominio secundario .ai ou outro. Pula se nao tiver." | `meusite.ai` |
| 18 | `crm.nadia.local` | "(Opcional) Dominio do seu CRM" | `crm.meusite.com` |
| 19 | `cliente.nadia.local` | "(Opcional) Subdominio exemplo de cliente" | `cliente1.meusite.com` |
| 20 | `cliente.nadia.local` | "(Opcional) Dominio de um cliente real (so pra exemplo)" | `cliente1.com.br` |
| 22 | `Nadia` | "Nome do seu produto/SaaS principal" | `Meu CRM` |
| 23 | `nadia` | "Slug do produto" | `meu-crm` |
| 24 | `Mentoria Nadia` | "(Opcional) Nome da sua mentoria" | `Mentoria X` |
| 25 | `Formacao Nadia IA` | "(Opcional) Nome da sua formacao/curso" | `Formacao X em IA` |
| 26 | `Comunidade Nadia` | "(Opcional) Nome da sua comunidade paga" | `Comunidade X` |
| 27 | `nadia2026` | "Senha admin pro agent-manager (TROCA depois pelo painel!). Default: ano+empresa." | `meusite2026` |
| 28 | `youruser` | "Seu username no GitHub" | `joaodev` |
...
User|VALOR_REAL_1
User|VALOR_REAL_2
youruser|VALOR_REAL_3
YOURUSER|VALOR_REAL_4
your-email@example.com|VALOR_REAL_5
User|VALOR_REAL_6
youruser|VALOR_REAL_7
YOURUSER|VALOR_REAL_8
PENDENTE|VALOR_REAL_9
PENDENTE_bot|VALOR_REAL_10
youruser|VALOR_REAL_11
YOUR_VPS_IP|VALOR_REAL_12
nadia.local|VALOR_REAL_13
Nadia|VALOR_REAL_14
nadia|VALOR_REAL_15
nadia2026|VALOR_REAL_16
youruser|VALOR_REAL_17
EOF
...
4. Hierarquia (Dono manda, Nadia orquestra, Juliana coordena, subagentes executam)
5. REGRA SUPREMA - PROTOCOLO DE CONVERSA 3 FASES (igual ao da Nadia)
6. ARQUITETURA DE ORQUESTRADORA (Nadia delega, nao executa)
...
## ETAPA 8 - SUBIR CLONE DO USER SDR (CONFIG PERSONALIZADO)
...
**Logs ao vivo:**
```bash
tail -f /opt/AGENTE/logs/agent.log         # Nadia Claude
tail -f /opt/AGENTE-bot/logs/bot.log       # Bot Python
pm2 logs agent-manager                      # agent-manager
journalctl -u AGENTE -f                     # systemd Nadia
```

**Restart:**
```bash
systemctl restart AGENTE          # restart Nadia
...
https://github.com/YOUR_GITHUB_USER/agente-claude-telegram-setup-alunos-student/issues
