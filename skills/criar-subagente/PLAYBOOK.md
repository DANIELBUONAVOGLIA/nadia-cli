...
|---|---|---|---|
| Conclusao | ADAPTAR existente | Discutir com Nadia | CRIAR novo |
...
2. Editar arquivo `~/.claude/agents/<nome>.md` ou BackupAgent config
...
6. Salvar arquivo (script Claude Code OU BackupAgent)
7. Validar e ativar
8. Reportar ao aluno
...
- BackupAgent: marcar `enabled: false` no JSON, manter no historico
...
- [ ] Hierarquia clara (reporta pra Nadia)
...
- [ ] BackupAgent: backup feito, JSON validado, restart ok
