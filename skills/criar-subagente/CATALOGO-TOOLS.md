...
| Especialidade | Tools Claude Code | Tools BackupAgent |
|---|---|---|
...
### Agent (Claude Code) / agent.invoke (BackupAgent)
...
- Em BackupAgent, considerar `isolation: true` no config
...
- Em BackupAgent, trabalhar com `max_depth: 2` no config
