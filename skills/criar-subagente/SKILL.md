...
description: Cria um subagente novo do zero (Claude Code ou BackupAgent), com personalidade, system prompt, tools restritas e nicho especifico. Acionar quando o aluno disser "cria um subagente pra X", "preciso de um agente especialista em Y", "monta um subagente novo de Z", "quero um SDR de estetica", "cria um copywriter de mercado financeiro", "monta um analista de dados", "adiciona um agente novo no time", "expande a equipe com mais um agente", "preciso de um especialista em <nicho>", ou qualquer pedido pra adicionar um novo membro a equipe Nadia (alem dos 13 padrao).
...
6. Arquivo salvo no destino correto (Claude Code OU BackupAgent)
7. Validacao e ativacao automatica

## Decisao: Claude Code ou BackupAgent?
...
- **BackupAgent**: edita `/root/.backup-agent/backup-agent.json` + cria workspace dedicado
...
2. **Hierarquia**: filial da Nadia, reporta pra Nadia. Nunca fala direto com Chefe sem passar pela Nadia.
...
Caminho do JSON: `/root/.backup-agent/backup-agent.json`
Caminho do workspace: `/root/.backup-agent/workspace-<nome>/`

Usar script: `scripts/criar-subagente-backup-agent.py`

```bash
python3 skills/criar-subagente/scripts/criar-subagente-backup-agent.py \
  --nome "sdr-estetica" \
  --descricao "SDR especialista em estetica..." \
...
O script:
1. Faz backup `backup-agent.json.bak.<timestamp>` antes de editar
2. Valida JSON antes de salvar (`json.loads`)
3. Cria workspace `/root/.backup-agent/workspace-<nome>/` com SOUL.md, MEMORY.md, BOOTSTRAP.md
4. Restart `systemctl restart backup-agent-gateway`
5. Roda `backup-agent doctor` no fim
...
- Restart obrigatorio: `systemctl restart backup-agent-gateway`
- Validar com: `backup-agent doctor`
...
- Em codigo (BackupAgent): chamada via gateway com agent_id="<nome>"
...
- `scripts/criar-subagente-backup-agent.py` — script BackupAgent
- `templates/subagent-template.md` — skeleton Claude Code
- `templates/subagent-soul-template.md` — skeleton SOUL.md BackupAgent
...
3. **Hierarquia clara**: subagente reporta pra Nadia, nao fala com Chefe direto
4. **Menos tools = mais seguro**: dar so o necessario
5. **Backup antes de editar JSON** (BackupAgent) — sempre, com timestamp
